...
<div class="fs-card">
  ...
  <div class="fs-chip"><span class="lbl">BENEFIT</span><span class="val">{format_eur(r.display_benefit)}</span></div>
  <div class="fs-chip"><span class="lbl">{r.gap_label.upper()}</span><span class="val">{format_pax(r.display_delta)} pax</span></div>
</div>
"""


def _airport_block(name: str | None, iata: str, country: str | None, continent: str) -> str:
    """Labeled rows for one airport, showing only fields present in the file."""
    rows = [("IATA", iata)]
    if name:
        rows.append(("Airport", html.escape(name)))
    if country:  # AIRPORTS.xlsx currently has no country data; shown if ever populated
        rows.append(("Country", html.escape(country)))
    rows.append(("Region", continent))
    items = "".join(
        f'<div class="row"><span class="k">{k}</span><span class="v">{v}</span></div>' for k, v in rows
    )
    return f'<div class="fs-airport">{items}</div>'
...
<div class="fs-detail-grid">
  ...
  <div style="grid-column:1">{_airport_block(r.origin_name, r.origin_iata, r.origin_country, r.origin_continent)}</div>
  <div style="grid-column:2">{_airport_block(r.dest_name, r.dest_iata, r.dest_country, r.dest_continent)}</div>
</div>
...
<div class="fs-detail-grid">
  ...
  <div class="k">{r.gap_label}</div><div class="v">{format_pax(r.display_delta)} pax</div>
  <div class="k">Benefit</div><div class="v">{format_eur(r.display_benefit)}</div>
</div>
"""
